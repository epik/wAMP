...
drag = $special.drag = {
	...
	setup: function(){
		...
		$event.add( this, START_EV, drag.init, data );
	...
	teardown: function(){
		...
		$event.remove( this, START_EV, drag.init );
	...
	init: function( event )
	{
		...
		var point = hasTouch ? event.originalEvent.changedTouches[0] : event;

		// the drag/drop interaction data
		var dd = event.data, results;
		
		// check the which directive
		if (( dd.which > 0) && (event.which != dd.which) && !(hasTouch))
			return; 
		...
		dd.pageX = point.pageX;
		dd.pageY = point.pageY;
		dd.dragging = null;
		...
		// This will already be bound if we have touch
		$event.add( document, MOVE_EV + " " + END_EV, drag.handler, dd );
	...
	handler: function( event )
	{
		...
		switch ( event.type ){
			...
			case !dd.dragging && MOVE_EV: 
				//  drag tolerance, x² + y² = distance²
				if (Math.pow(  event.pageX-dd.pageX, 2 ) + 
							Math.pow(  event.pageY-dd.pageY, 2 ) < Math.pow( dd.distance, 2 ) ) 
			...
			case MOVE_EV: 
				if ( dd.dragging ){
					...
					event.type = END_EV; // helps "drop" handler behave
				}
			// mouseup, stop dragging
			case END_EV: 
				$event.remove( document, 
							  MOVE_EV + " " +  END_EV, drag.handler ); // remove page events
				if ( dd.dragging ){
					if ( dd.drop !== false && $special.drop ) 
