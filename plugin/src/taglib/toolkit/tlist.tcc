...
public:
  ...
  ListPrivate(const eastl::list<TP> &l) : ListPrivateBase(), list(l) {}
  void clear() {
    list.clear();
  }
  eastl::list<TP> list;
...
public:
  ...
  void clear() {
    if(autoDelete) {
      typename eastl::list<TP *>::const_iterator it = list.begin();
      for(; it != list.end(); ++it)
        delete *it;
    }
    list.clear();
...
{
  return eastl::find(d->list.begin(), d->list.end(), value);
}
...
{
  return eastl::find(d->list.begin(), d->list.end(), value) != d->list.end();
}
