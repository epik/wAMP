...
struct FileNameHandle : public eastl::string
{
  FileNameHandle(FileName name) : eastl::string(name) {}
...
{
public:
  FilePrivate(FileName fileName, bool bReadOnly);
...
File::FilePrivate::FilePrivate(FileName fileName, bool bReadOnly) :
  file(0),
  name(fileName),
  readOnly(true),
  valid(true),
...
File::File(FileName file, bool readOnly)
{
  d = new FilePrivate(file, readOnly);
}
...
void File::writeBlock(const ByteVector &data)
{
  ...
  size_t i = fwrite(data.data(), sizeof(char), data.size(), d->file);
...
void File::insert(const ByteVector &data, ulong start, ulong replace)
{
  ...
  fprintf(stderr, "Let's get to the bottom of this datasize:%i replace:%i\n",
		  	  	  data.size(),
		  	  	  replace);
