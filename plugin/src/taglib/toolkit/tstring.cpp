...
eastl::string String::to8Bit(bool unicode) const
{
  eastl::string s;
  s.resize(d->data.size());

  if(!unicode) {
	 eastl::string::iterator targetIt = s.begin();
...
const char *String::toCString(bool unicode) const
{
  ...
  eastl::string buffer = to8Bit(unicode);
...
String &String::operator=(const eastl::string &s)
{
  if(d->deref())
    delete d;
