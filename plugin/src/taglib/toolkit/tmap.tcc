...
public:
  ...
  MapPrivate(const eastl::map<class KeyP, class TP>& m) : RefCounter(), map(m) {}
  eastl::map<class KeyP, class TP> map;
#else
  MapPrivate(const eastl::map<KeyP, TP>& m) : RefCounter(), map(m) {}
  eastl::map<KeyP, TP> map;
#endif
};
